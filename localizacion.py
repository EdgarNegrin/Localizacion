...
def localizacion(balizas, real, ideal, centro, radio, mostrar=0):
  ...
  imagen = []
  steps = 0.1
  for y in np.arange(0, 2*radio, steps):
    imagen.append([])
    for x in np.arange(0, 2*radio, steps):
      ideal.set(centro[0] - radio + x, centro[1] - radio + y, centro[2]) 
      meassProb = ideal.measurement_prob(medidasReales, balizas)
      imagen[-1].append(meassProb)
      if meassProb < menor:
        menor = meassProb
        poseFinal = ideal.pose()
  ideal.set(*poseFinal)
...
localizacion(objetivos, real, ideal, ideal.pose(), 6, 1) #Localizacion inicial
pose = ideal.pose()

for punto in objetivos:
  while distancia(tray_ideal[-1],punto) > EPSILON and len(tray_ideal) <= 1000:
    
    probUmbral = ideal.measurement_prob(real.sense(objetivos), objetivos) #Diferencia entre ideal y real
    if probUmbral > 1:
      localizacion(objetivos, real, ideal, ideal.pose(), 2) #Radio mayor que el umbral para poder alcanzar al real
